...
using phase transition detection.
...
def main():
    ...
    if not args.output:
        ...
        args.output = f"streamgraph_phase_data_{timestamp}.json"
    ...
    if args.plot and not results.get("failed", False):
        try:
            print("Generating phase-tracking streamgraph visualization...")
            fig = generate_phase_tracking_streamgraph(results)
            
            # Save figure
            plot_path = os.path.splitext(args.output)[0] + "_streamgraph_phase.png"
