...
if page == "Dashboard":
        ...
        if staggered_can_run:
            with st.spinner(f"Running staggered enrollment simulation with ~{expected_patients} patients over {duration_years} years..."):
                ...
                if "error" in staggered_results or staggered_results.get("failed", False):
                    ...
                    if enrollments:
                        # Show patient enrollment with R visualization
                        ...
                        # Try to use R visualization if available
                        # Use our improved visualization approach
                        # Use our improved visualization approach that shows matplotlib first
                        # then replaces with R visualization when available
                        try:
                            from streamlit_app.improved_visualization import render_enrollment_visualization
                            render_enrollment_visualization(enroll_df)
                        except Exception as e:
                            # If there's an error with the improved visualization, fall back to simple matplotlib plot
                            print(f"Error in improved visualization: {e}")
                            # Group by month
    ...
    if has_standard_simulation or has_staggered_simulation:
        ...
        if has_staggered_simulation:
            with tabs[tab_index]:
                staggered_results = st.session_state["staggered_results"]
                patient_histories = staggered_results.get("patient_histories", {})
