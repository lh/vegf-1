"""
TreatAndExtend adapter for EnhancedDES framework.

This module provides an adapter that allows the TreatAndExtendDES class to use
the enhanced DES framework while maintaining the same interface and behavior.
It follows the adapter design pattern to bridge the gap between the original
TreatAndExtendDES implementation and the new event-driven EnhancedDES framework.

Classes
-------
TreatAndExtendAdapter
    Adapter class that implements the TreatAndExtendDES interface using EnhancedDES
"""
...
from .vision_models import create_vision_model
from .enhanced_discontinuation_manager import EnhancedDiscontinuationManager
from .clinician import ClinicianManager

# Configure logging
logger = logging.getLogger(__name__)

class TreatAndExtendAdapter:
    """
    Adapter for TreatAndExtendDES to use the EnhancedDES framework.
    
    This class wraps the EnhancedDES framework and exposes the same interface
    as the original TreatAndExtendDES class. It translates between the old event
    structure and the new standardized event structure.
    ...
    def __init__(self, config_name="eylea_literature_based"):
        """
        Initialize the adapter with the specified configuration.
        ...
        # Initialize enhanced DES
        self.simulation = EnhancedDES(self.config)
        
        # Register event handlers
        self.simulation.register_event_handler("visit", self._handle_visit)
        self.simulation.register_event_handler("treatment_decision", self._handle_treatment_decision)
        self.simulation.register_event_handler("add_patient", self._handle_add_patient)
    ...
    def run(self):
        ...
        """
        print("Initializing treat-and-extend DES simulation with adapter...")
        
        # Run the simulation
        end_date = self.config.start_date + timedelta(days=self.config.duration_days)
        results = self.simulation.run(end_date)
        
        # Extract patient histories from results
    ...
    def _handle_add_patient(self, event: Event):
        ...
        self.simulation.clock.schedule_event(Event(
            time=event.time,
            event_type="visit",
            patient_id=patient_id,
    ...
    def _handle_treatment_decision(self, event: Event):
            ...
            if fluid_detected:
                ...
                if should_discontinue:
                    ...
                    for monitoring_event in monitoring_events:
                        if monitoring_event["time"] <= self.simulation.clock.end_date:
                            self.simulation.clock.schedule_event(Event(
        ...
        if patient.state["treatment_status"]["active"] and next_visit_time <= self.simulation.clock.end_date:
    ...
    def _get_patient(self, patient_id):
        """
        Get patient state from the simulation.
        ...
        Optional[PatientState]
            Patient state or None if not found
        """
        return self.simulation.patients.get(patient_id)
    
    def _print_statistics(self):
        """
        Print simulation statistics.
        
        Prints a summary of the simulation statistics including:
        - Patient counts
        - Visit counts
        ...
        total_patients = len(self.simulation.patients)
        ...
        print("\nPatient Summary:")
        print("-" * 20)
        total_visits = sum(len(patient.state.get("visit_history", [])) for patient in self.simulation.patients.values())
...
# Entry point function for running the adapter
def run_treat_and_extend_des_adapter(config_name="eylea_literature_based", verbose=False):
    """
    Run a DES simulation with the treat-and-extend protocol using the adapter.
    ...
    Dict[str, List[Dict]]
        Dictionary mapping patient IDs to their visit histories
    """
    # Create adapter and run simulation
    print(f"Running TreatAndExtend DES simulation with adapter using config: {config_name}")
    adapter = TreatAndExtendAdapter(config_name)
    patient_histories = adapter.run()
    ...
    if verbose:
        ...
        df.to_csv('treat_and_extend_des_adapter_data.csv', index=False)
        print(f"Saved data for {len(patient_histories)} patients to treat_and_extend_des_adapter_data.csv")
