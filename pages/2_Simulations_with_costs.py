"""
Simulations - Run new simulations with optional cost tracking.
"""
...
# Import our new component modules with cost support
...
from ape.components.simulation_ui import (
    render_runtime_estimate, render_control_buttons, 
    render_recent_simulations, clear_preset_values
)
from ape.components.simulation_ui_v2_with_costs import (
    render_enhanced_parameter_inputs_with_costs, render_preset_buttons_v2,
...
# Get parameters including cost configuration
engine_type, recruitment_params, seed, cost_config = render_enhanced_parameter_inputs_with_costs()
...
if st.session_state.get('simulation_running', False):
    ...
    cost_config = st.session_state.get('cost_config')
    ...
    try:
        ...
        # Create runner with cost configuration if enabled
        if cost_config and cost_config.get('enabled'):
        ...
        if recruitment_params['mode'] == 'Fixed Total':
            ...
            if cost_config and cost_config.get('enabled'):
                results = runner.run(
                    ...
                    drug_type=cost_config.get('drug_type')
                )
            else:
                results = runner.run(
                    engine_type=recruitment_params['engine_type'],
        ...
        simulation_data = {
            ...
            'audit_trail': runner.audit_log,
            'cost_tracking': cost_config if cost_config and cost_config.get('enabled') else None
        }
