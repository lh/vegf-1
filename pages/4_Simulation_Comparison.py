...
def create_tufte_injection_plot(ax, injections, stats, color='blue', bins=None, label='', show_label=True):
    """Create a Tufte-style injection frequency histogram."""
    # Create histogram
    n, bins_plot, patches = ax.hist(injections, bins=bins, 
                                   color=color, alpha=0.4, 
                                   edgecolor='none')
    
    # Add mean line with direct label
    ax.axvline(stats['mean'], color=color, linestyle='-', 
              linewidth=2, alpha=0.8)
    
    # Add label and mean value
    if show_label and label:
        # Add simulation name in top left
        ax.text(0.05, 0.95, label, 
               transform=ax.transAxes,
               fontsize=10, color=color, ha='left', va='top',
               weight='bold')
    
    # Add mean value near the mean line
    ax.text(stats['mean'] + 0.5, ax.get_ylim()[1] * 0.85, 
    ...
    # Set custom x-axis ticks (3, 7, 13, 19)
    ax.set_xticks([3, 7, 13, 19])
    ax.set_xticklabels(['3', '7', '13', '19'])
    
    # Set x-axis to start at 1
    ax.set_xlim(left=1)
    
    # Labels
    ax.set_xlabel('Total Injections per Patient', fontsize=10, color='#333333')
    ax.set_ylabel('Number of Patients', fontsize=10, color='#333333')
    
    # Minimal grid on y-axis only
...
if view_mode == "Side-by-Side":
    ...
    # Create both plots with identical parameters
    create_tufte_injection_plot(ax_a, injections_a, stats_a, color='blue', bins=bins, 
                               label=sim_a['memorable_name'])
    create_tufte_injection_plot(ax_b, injections_b, stats_b, color='orange', bins=bins,
                               label=sim_b['memorable_name'])
    ...
    # Add mean lines with direct labels
    ...
    ax.text(0.05, 0.95, sim_a['memorable_name'], 
           transform=ax.transAxes,
           fontsize=10, color='blue', ha='left', va='top',
           weight='bold')
    ax.text(0.05, 0.90, sim_b['memorable_name'], 
           transform=ax.transAxes,
           fontsize=10, color='orange', ha='left', va='top',
           weight='bold')
    
    # Direct labels for means
    ax.text(stats_a['mean'] + 0.5, ax.get_ylim()[1] * 0.85, 
           f"Mean: {stats_a['mean']:.1f}", 
           fontsize=9, color='blue', ha='left')
    ax.text(stats_b['mean'] + 0.5, ax.get_ylim()[1] * 0.75, 
           f"Mean: {stats_b['mean']:.1f}", 
    ...
    # Plot difference
    ax.plot(x_range, diff, '#666666', linewidth=2)
    ...
    # Zero line
    ax.axhline(y=0, color='#333333', linestyle='-', alpha=0.5, linewidth=0.8)
    
    # Add mean lines
    mean_diff = stats_b['mean'] - stats_a['mean']
    ax.axvline(stats_a['mean'], color='blue', linestyle='-', alpha=0.6, linewidth=1.5)
    ax.axvline(stats_b['mean'], color='orange', linestyle='-', alpha=0.6, linewidth=1.5)
    
    # Add labels for means
    ax.text(stats_a['mean'] - 0.5, ax.get_ylim()[1] * 0.95, 
           f"{sim_a['memorable_name']}\nMean: {stats_a['mean']:.1f}", 
           fontsize=9, color='blue', ha='right', va='top')
    ax.text(stats_b['mean'] + 0.5, ax.get_ylim()[1] * 0.95, 
           f"{sim_b['memorable_name']}\nMean: {stats_b['mean']:.1f}", 
           fontsize=9, color='orange', ha='left', va='top')
    
    # Add difference annotation
    ax.text(0.95, 0.95, f"Mean difference: {mean_diff:+.1f}",
    ...
    # Labels
    ax.set_xlabel('Total Injections per Patient', fontsize=10, color='#333333')
    ax.set_ylabel('Density Difference (B - A)', fontsize=10, color='#333333')
