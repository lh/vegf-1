...
try:
    ...
    if protocol_type == "time_based":
        # Check if time-based protocol has clinical improvements
    ...
    # Clinical Improvements tab for time-based protocols (tab5)
    if protocol_type == "time_based" and has_improvements:
        with tab5:
            st.subheader("Clinical Improvements")
            ...
            if st.session_state.get('edit_mode', False) and selected_file.parent == TEMP_DIR:
                ...
                if is_enabled:
                    ...
                    st.markdown("---")
                    st.info("""
                    **Impact of Clinical Improvements (Time-Based Model):**
                    - More realistic patient trajectories with fortnightly disease updates
                    - Better representation of real-world treatment patterns
                    - Enhanced accuracy for long-term projections
                    - Compatible with time-based disease progression
                    """)
                    
                else:
                    st.warning("**Clinical Improvements: DISABLED**")
                    st.caption("This time-based protocol uses the standard simulation model without clinical improvements.")
                    st.caption("To enable improvements, edit a temporary copy of this protocol.")
