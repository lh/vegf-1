"""Enhanced simulation UI components with recruitment mode selection and cost tracking."""
...
def render_cost_tracking_config() -> Optional[Dict[str, Any]]:
    """
    Render cost tracking configuration UI.
    
    Returns:
        Dictionary with cost tracking configuration or None if disabled
    """
    # Cost tracking toggle
    enable_costs = st.checkbox(
        "Enable Cost Tracking",
        value=st.session_state.get('enable_cost_tracking', False),
        help="Track costs and workload during simulation"
    )
    
    st.session_state.enable_cost_tracking = enable_costs
    
    if not enable_costs:
        return None
        
    # Show cost configuration widget
    with st.container():
        cost_widget = CostConfigurationWidget()
        drug_type, drug_cost, cost_config = cost_widget.render()
        
        # Return cost tracking configuration
        return {
            'enabled': True,
            'drug_type': drug_type,
            'drug_cost': drug_cost,
...
def render_recruitment_parameters_with_costs() -> Tuple[Dict[str, Any], Optional[Dict[str, Any]]]:
    """
    Render recruitment parameters with smart dependency and cost tracking.
    
    Returns:
        Tuple of (recruitment_params, cost_config)
    """
    # First render recruitment parameters as before
    recruitment_params = render_recruitment_parameters_original()
    
    # Then render cost tracking configuration
    st.markdown("---")
    st.subheader("💰 Economic Analysis")
    cost_config = render_cost_tracking_config()
    
    return recruitment_params, cost_config


def render_recruitment_parameters_original() -> Dict[str, Any]:
    """
    Original render_recruitment_parameters function.
    ...
    stored_duration = st.session_state.get('duration_years',
                      st.session_state.get('preset_duration', 2))
    ...
    with col3:
        def on_duration_change():
            # Duration change doesn't change the last edited field
    ...
    with col4:
        # Two-column layout for seed and button
...
def render_enhanced_parameter_inputs_with_costs() -> Tuple[str, Dict[str, Any], int, Optional[Dict[str, Any]]]:
    """
    Render all simulation parameters including recruitment settings and cost tracking.
    
    Returns:
        Tuple of (engine_type, recruitment_params, seed, cost_config)
    """
    ...
    # Recruitment parameters and cost configuration
    recruitment_params, cost_config = render_recruitment_parameters_with_costs()
    ...
    return engine_type, recruitment_params, seed, cost_config
